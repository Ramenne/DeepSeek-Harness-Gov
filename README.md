...
- 从 DSH 动态读取已允许的模型、推理强度、Agent preset 与权限预设；当前公网页面仅开放已经实测可用的 MiniMax 提供商。
...
插件服务端配置保存在 `$DSH_HOME/gov-portal.json`；未设置 `DSH_HOME` 时回退到 `~/.dsh/gov-portal.json`。浏览器界面配置保存在当前站点的 `localStorage`。监听端口等服务参数修改后需要重启 DSH。
...
在政务平台业务大厅输入上述斜杠命令时，门户会将其直接交给 DSH 的命令服务执行，不会把命令文本作为普通提示词发送给模型。
